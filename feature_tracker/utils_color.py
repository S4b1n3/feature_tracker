...
def generate_trajectory(num_frames, speed, start_angle, first_half=True):
    trajectory_hsv = []
    trajectory_rgb = []
    for frame in range(num_frames):
        ...
        hsv = angle_to_hsv_half(time, first_half=first_half)
...
def generate_all_colors(nb_images, nb_frames, nb_objects, first_half, speed=1/64.0):
    ...
    for img in range(nb_images):
        for obj in range(nb_objects):
            ...
            trajectory_hsv, trajectory_rgb = generate_trajectory(nb_frames, speed, start_angle, first_half=first_half)
