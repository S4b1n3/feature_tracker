...
def save_checkpoint(state, is_best, results_folder):
    ...
    try:
        ...
        pass
    filename = save_folder + 'model_val_acc_{0:04d}_val_loss{1:04f}_epoch_{2:02d}_checkpoint.pth.tar'.format(
        int(state['best_acc']), float(state['best_loss']), state['epoch'])
